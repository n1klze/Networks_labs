...
class ClientHandler(Thread):
    def __init__(self, conn, addr):
        ...
        self.conn = conn
        self.addr = "{0}:{1}".format(addr[0], str(addr[1]))
    ...
    def run(self):
        ...
        with open("uploads/" + basename(filename), "wb") as file:
            ...
            while read_bytes < file_size:
                ...
                if current_time - last_time > INTERVAL:
                    ...
                    avg_speed = (
                        read_bytes / 1024 / 1024 / (current_time - start_time) * 10**9
                    )
                    print(
                        self.addr,
                        ":",
            ...
            if 0 < time.time_ns() - start_time <= INTERVAL:
                avg_speed = (
                    read_bytes / 1024 / 1024 / (time.time_ns() - start_time) * 10**9
                )
        ...
        print(
            ...
            ":",
            "Success"
            if getsize("uploads/" + basename(filename)) == file_size
            else "Failure: received: {0} expected: {1}".format(
                str(read_bytes), str(file_size)
            ),
        )
